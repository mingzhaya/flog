...
def index():
    ...
    if form.validate_on_submit():
        ...
        db.session.add(post)
        db.session.commit()
        flash('You successfully made a post!')
...
def explore():
    ...
    return render_template('index.html', title='Exploration', posts=posts.items,
                            next_url=next_url, prev_url=prev_url)
...
def register():
    ...
    if form.validate_on_submit():
        ...
        db.session.add(user)
        db.session.commit()
        flash('You have registered as {}.'.format(form.username.data))
        return redirect(url_for('login'))
